...
netlify deploy --prod   # deploy manual (após npm run build)
```
...
**`servicos`** — `tipo_servico`: `servico`, `peca`, ou `terceirizado`. Controla a visibilidade do campo fornecedor na UI.
...
**Branches:** `main` (branch local de desenvolvimento); `master` é o default no GitHub (pendente de troca).
