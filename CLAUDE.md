...
**`configuracoes`** — chave/valor. Seed: `validade_orcamento_dias = '30'`. Chaves opcionais de imposto: `aliquota_servico` / `aliquota_peca` (em pontos percentuais); ausentes ⇒ o Dashboard usa `ALIQUOTA_FALLBACK`.
...
No modo Editar de `OrdemServico.jsx` (botão "Editar" no modal da OS), cada item da tabela `itensEdit` tem `quantidade` e `preco_cobrado` editáveis inline — vale para os três tipos (`servico`, `peca`, `terceirizado`). Cada linha usa `uid` estável como React key (`i.id` para existentes, `novo-N` via `uidSeq` para adicionados) e o input de preço é controlado por um campo espelho `precoTexto`; sem isso, remover um item deixava o preço do vizinho dessincronizado. `salvarEdicao()` faz três operações em `os_servicos`: `update` dos existentes (via `Promise.all`, com checagem de erro), `insert` dos novos e `delete` dos removidos; **só grava `ordens_servico.valor_total` se nenhum update falhou** (senão o total divergiria do banco).

### Dashboard: KPIs filtrados no servidor (teto de 1000 linhas)
`Dashboard.jsx` faz **duas** queries em `os_servicos`, ambas filtradas por período **no servidor** (não client-side): faturamento é reconhecido na **conclusão** (`ordens_servico.concluida_em`), compra de peça na **entrada** (`aberta_em`). Motivo do filtro server-side: um select sem `where` estoura o **teto de 1000 linhas do PostgREST** e truncava silenciosamente faturamento/imposto/ticket. Todas as queries do dashboard rodam num único `Promise.all`. Imposto usa alíquotas de `configuracoes` (`aliquota_servico`/`aliquota_peca`, em pontos percentuais) com fallback `ALIQUOTA_FALLBACK = { servico: 10.56, peca: 7.19 }` (Simples faixa 3 — ver `docs/simples-faixas.md`).
